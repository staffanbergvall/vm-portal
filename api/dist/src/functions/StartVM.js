...
/**
 * StartVM Function - Starts a specific VM
 */
...
async function StartVM(request, context) {
    ...
    try {
        ...
        context.log(`Starting VM: ${vmName} in ${azureAuth_1.VM_RESOURCE_GROUP}`);
        ...
        context.log(`VM ${vmName} started successfully by ${userEmail}`);
        return {
            status: 200,
            jsonBody: {
                success: true,
        ...
        context.error(`StartVM error for ${vmName}:`, errorMessage);
        return {
            status: 500,
            jsonBody: {
...
functions_1.app.http('StartVM', {
    methods: ['POST'],
    authLevel: 'anonymous', // Auth handled by SWA
    route: 'vms/{vmName}/start',
    handler: StartVM
});
//# sourceMappingURL=StartVM.js.map